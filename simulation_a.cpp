...
struct CameraObjectData : public FactorData<16, true> {
    ...
    std::array<Eigen::VectorD<2>, num_points> image_points;
};
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_points; ++i) {
        auto image_point = camera_project(
            ...
            se3_world_object,
            object_points[i]);
        camera_object_data.image_points[i] = image_point;
    }
    ...
    const auto factor = graph.add_factor(camera_object_data);
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // relinearize camera object factors
        for (auto [_, data, lin_point, info_matrix, info_vector] :
            ...
            double error2 = camera_project_factor(
                ...
                object_points.size(),
                object_points.data(),
                data.image_points.data(),
                &info_matrix,
                &info_vector);
            std::cout << "projection error2 " << error2 << "\n";
        }

        // todo: relinearize the other factors
        // {
        //    ...
        // }
